...
if "Distributions" in tabs_list:
    with tabs[tab_idx]:
        ...
        sel_num_cols = st.multiselect(
            "Select columns", col_types["numeric"],
            default=col_types["numeric"][:4], key="dist_cols",
        )
        if sel_num_cols:
...
if "Categories" in tabs_list:
    with tabs[tab_idx]:
        ...
        sel_cat_cols = st.multiselect(
            "Select columns", col_types["categorical"],
            default=col_types["categorical"][:4], key="cat_cols",
        )
        if sel_cat_cols:
            for i in range(0, len(sel_cat_cols), 2):
                ...
                for j, col_name in enumerate(sel_cat_cols[i:i + 2]):
                    ...
                    with cols[j]:
                        ...
                        if len(vc) <= 8:
                            fig = px.pie(
                                vc, values="Count", names=col_name,
                                hole=0.45, color_discrete_sequence=COLORS,
                                template="plotly_dark",
                            )
                        else:
                            fig = px.bar(
                                vc, x="Count", y=col_name, orientation="h",
                                color="Count", color_continuous_scale=GRADIENT_PURPLE,
                                template="plotly_dark",
                            )
...
if "Correlations" in tabs_list:
    with tabs[tab_idx]:
        ...
        if len(num_df.columns) >= 2:
            ...
            fig = px.imshow(
                corr.values, x=corr.columns.tolist(), y=corr.index.tolist(),
                color_continuous_scale=GRADIENT_DIVERGING,
                zmin=-1, zmax=1, template="plotly_dark", aspect="auto", text_auto=".2f",
            )
